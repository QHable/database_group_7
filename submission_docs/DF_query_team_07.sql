-- revenue in specific month  (to give to the accountant)
...
SELECT 'order' AS "type", 
	(c_or_p.product_id || ' ' || c_or_p.client_id) AS "id", 
	c_or_p.order_date AS "date", 
	(c_or_p.order_amount * pro.product_price) AS "money earned"
FROM group_7.client_orders_product c_or_p
	INNER JOIN group_7.products pro ON (c_or_p.product_id = pro.id)
WHERE EXTRACT(year from c_or_p.order_date) = '2025' 
	AND EXTRACT(month from c_or_p.order_date) = '01'
ORDER BY date;
...
SELECT p.animal_breed, 
	COUNT(p.animal_species) AS no_appointments
FROM group_7.appointment AS a
...
SELECT app.appointment_date, wo.employee_name, 
	room.id as "room", 
	(app.start_time || '-'|| app.end_time) as "time", 
	app.appointment_type, 
	app.description, 
	app.patient_id as "patient", app.id 
